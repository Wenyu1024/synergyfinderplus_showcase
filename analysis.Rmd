...
get_triocomb_data <- function(id){
  ...
  complete <- bind_rows(tmp4, tmp5, tmp6) %>% 
    ...
    mutate(conc1= as.numeric(conc1)) %>% 
    mutate(conc2= as.numeric(conc2)) %>%  
    select(BlockId,Replicate,drug1,drug2,drug3,conc1,conc2,conc3,Value,conc_unit1,conc_unit2,conc_unit3,  RowSid, ColSid)
...
tmp =get_triocomb_data(id = 12255) # compare this ouput with Shuyu's code output
```
